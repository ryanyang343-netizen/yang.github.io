# -*- coding: utf-8 -*-
import binascii
import struct
import base64
import json
import os
from Crypto.Cipher import AES

song_filepath=input()

def dump(file_path):
    core_key = binascii.a2b_hex("687A4852416D736F356B496E62617857")
    meta_key = binascii.a2b_hex("2331346C6A6B5F215C5D2630553C2728")
    unpad = lambda s: s[0:-(s[-1] if type(s[-1]) == int else ord(s[-1]))]

    with open(file_path, 'rb') as f:
        header = f.read(8)
        assert binascii.b2a_hex(header) == b'4354454e4644414d'  # "CTENFDAM"
        f.seek(2, 1)

        # --- 1. 读取并解密 key 区域 ---
        key_length = struct.unpack('<I', f.read(4))[0]
        key_data = bytearray(f.read(key_length))
        for i in range(len(key_data)):
            key_data[i] ^= 0x64
        cryptor = AES.new(core_key, AES.MODE_ECB)
        key_data = unpad(cryptor.decrypt(bytes(key_data)))[17:]
        key_box = bytearray(range(256))
        key_length = len(key_data)
        last_byte = c = key_offset = 0
        for i in range(256):
            swap = key_box[i]
            c = (swap + last_byte + key_data[key_offset]) & 0xff
            key_offset = (key_offset + 1) % key_length
            key_box[i], key_box[c] = key_box[c], swap
            last_byte = c

        # --- 2. 读取 meta 区域 ---
        meta_length = struct.unpack('<I', f.read(4))[0]
        meta_data = bytearray(f.read(meta_length))
        for i in range(len(meta_data)):
            meta_data[i] ^= 0x63
        meta_data = base64.b64decode(bytes(meta_data)[22:])
        cryptor = AES.new(meta_key, AES.MODE_ECB)
        meta_data = json.loads(unpad(cryptor.decrypt(meta_data)).decode('utf-8')[6:])
        music_format = meta_data['format']

        # --- 3. 跳过封面、CRC 等 ---
        f.seek(9, 1)
        image_size = struct.unpack('<I', f.read(4))[0]
        f.seek(image_size, 1)

        # --- 4. 解密音频流 ---
        out_path = os.path.splitext(file_path)[0] + '.' + music_format
        with open(out_path, 'wb') as m:
            chunk_size = 0x8000
            while True:
                chunk = bytearray(f.read(chunk_size))
                if not chunk:
                    break
                for i in range(len(chunk)):
                    j = (i + 1) & 0xff
                    chunk[i] ^= key_box[(key_box[j] + key_box[(key_box[j] + j) & 0xff]) & 0xff]
                m.write(chunk)
        return out_path

if __name__ == '__main__':
    filepath = rf"D:\CloudMusic\VipSongsDownload\{song_filepath}.ncm"
    result = dump(filepath)
    print("✅ 转换完成：", result)